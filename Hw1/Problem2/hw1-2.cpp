			...
			for (int i = 0; i < n; i++)
			{
				if (stock_vec[i].get_owner_name() == temp)
				{
					...
					while (temp != "1" && temp != "2" && temp != "3")
					{
						...
						cin >> temp;
						if (temp == "1") {stock_vec[i].buy();}
						else if (temp == "2") {stock_vec[i].sell();}
						else if (temp == "3") {stock_vec[i].show_stock();}
						else {cout << "wrong choice. please enter again." << endl;}
					}
				}
			}
			if (flag) {cout << "No this person. please enter again" << endl;}
		}
	}
	return 0;
}